...
public class EipCheckSewagePipeLinesLevelsActivity extends AppCompatActivity {
	...
	public void _PrintHelper(final View _view) {
		...
		WebView webView = new WebView(EipCheckSewagePipeLinesLevelsActivity.this);
		webView.loadDataWithBaseURL(null, htmlContent.toString(), "text/html", "UTF-8", null);
		webView.setWebViewClient(new WebViewClient() {
			@Override
			public void onPageFinished(WebView view, String url) {
				PrintManager printManager = (PrintManager) EipCheckSewagePipeLinesLevelsActivity.this.getSystemService(PRINT_SERVICE);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (id == R.id.action_help) {
			new androidx.appcompat.app.AlertDialog.Builder(this)
                ...
                "-----‐---------------------------------\n" +
                "استلام منسوب ظهر مواسير لفرعة صرف صحي أو خط طرد أو خط مياه أو خط ري أو خط حريق بمعلومية منسوب ميه الماسورة ببداية الفرعة أو الخط والميل التصميمي للفرعة أو الخط :\n" +
                "أضافة خط أو فرعة\n" +
                "جديدة")
